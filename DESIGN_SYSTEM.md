...
- **foundations** — Primitives: `foundations.color.*`, `foundations.radius.*`, `foundations.shadow.*`, `foundations.breakpoints.*`, etc. Raw values and layout/spacing scales. Token set: **primitives/foundations**. Layout scale sets under **layoutScale/**:
  ...
  - **layoutScale/web.mobile** / **layoutScale/web.tablet** — web layout scales for small / medium viewports, derived from the base scale (spacing ~0.75× base on mobile, ~midpoint on tablet).
  - **layoutScale/ios.phone** / **layoutScale/ios.tablet** — iOS-specific layout scales for iPhone / iPad. Use these when designing **native** iOS layouts so you can tune spacing separately from web while keeping the same semantic naming.
...
- **Typography scale web desktop** — **typographyScale/web.desktop** holds **t-shirt names** (xxs, xs, sm, …) as **refs** to base `type-1`…`type-9`, same pattern as layoutScale/web.desktop. Use in Figma for desktop type scale; values resolve from base so they can’t drift.
- **Figma rendering snapshots** — **typographyScale/snapshot.*** Where applicable, snapshots **map back to the numeric primitive scale** (type-1…type-9). **typographyScale/snapshot.web.desktop** uses refs like `{fontSize.type-1}`…`{fontSize.type-9}` (xxs→type-1 … 4xl→type-9) so the chain stays single-source. Tablet/mobile snapshots keep **computed** values (~0.9×, ~0.8× base). iOS snapshots are left untouched. Edit only the base scale; re-run the build to sync.

**Fluid scale for code (`clamp()`)**

- **typographyScale/web.fluid** `fontSize.*` (t-shirt keys): **max** maps back to the primitive scale (`{fontSize.type-1}`…`{fontSize.type-9}`); **min** stays computed (mobile-scale rem). Build script writes min from base and writes max as refs so fluid stays single-source.
- Code can map to `clamp(minRem, preferred, maxRem)` using **primitives/foundations** breakpoints.
...
- Never ship snapshot tokens to production code; exports and adapters should read from **typographyScale/base** / **typographyScale/web.fluid** and `web.typography.*` / `ios.typography.*` instead.
