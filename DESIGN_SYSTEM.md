...
**Token set names (top-level keys in tokens.json):** `primitives`, `typography.foundations`, `scale.layout.base`, `scale.layout.web.desktop`, `scale.layout.web.mobile`, `scale.layout.web.tablet`, `scale.layout.ios.phone`, `scale.layout.ios.tablet`, `scale.typography.base`, `scale.typography.web.fluid`, `scale.typography.snapshot.web.desktop`, `scale.typography.snapshot.web.mobile`, `scale.typography.snapshot.web.tablet`, `scale.typography.snapshot.ios.phone`, `scale.typography.snapshot.ios.tablet`, `semantics.web`, `semantics.ios`, `semantics.color.web.light`, `semantics.color.web.dark`, `semantics.color.ios.light`, `semantics.color.ios.dark`.

- **primitives** — Raw values: `primitives.color.*`, `primitives.radius.*`, `primitives.shadow.*`, `primitives.breakpoints.*`, etc. Layout/spacing scales live in **scale.layout**:
  - `scale.layout.base` — canonical desktop baseline. **Primitive naming**: prefix + number; **use 0 only when the value is 0**. Spacing: `space-0`…`space-10` (space-0 = 0, then 0.25rem…8rem). Container/lineLength/media/iconSize have no zero value, so they start at 1: `container-1`…`container-6`, `lineLength-1`…`lineLength-3`, `media-1`…`media-3`, `iconSize-1`…`iconSize-4`. Viewport sets use t-shirt/semantic names and reference base.
  - `scale.layout.web.desktop` — **aliased to base** via references; uses t-shirt names (xs, sm, md, …) and points at base (e.g. `spacing.xs` → `{spacing.space-1}`).
  - `scale.layout.web.mobile` / `scale.layout.web.tablet` — web layout scales for small / medium viewports, derived from the base scale (spacing ~0.75× base on mobile, ~midpoint on tablet).
  - `scale.layout.ios.phone` / `scale.layout.ios.tablet` — iOS-specific layout scales for iPhone / iPad. Use these when designing **native** iOS layouts so you can tune spacing separately from web while keeping the same semantic naming.
...
- **Web desktop** — `scale.layout.web.desktop` is reference-only; every value points at base via **short refs** (e.g. `{spacing.space-1}`, `{layout.container.container-1}`) so Tokens Studio can accept them. Resolve via `scale.layout.base.` prefix. Exports and Tailwind read base and output t-shirt names.
...
- **typography (canonical)** — `typography.foundations` (font families, weights), `scale.typography.base` (type scale in rem), and `scale.typography.web.fluid` (min/max for clamp). Base uses the same **prefix + number** convention: `fontSize.type-1`…`type-9` (no type-0; no zero size). Build script maps type-1…type-9 → xxs…4xl for Tailwind; snapshots use t-shirt names and ref or derive from base. Web semantics under `semantics.web.typography.*`; generic names (heading-1, body, caption) are adapter-only.

### Typography snapshots, fluid scale, and canonical

- **Canonical type scale** — `scale.typography.base.fontSize` uses **type-1…type-9** (0.563rem … 5.61rem). Same rule as other scales: **0 only when value is 0**; type sizes start at 1.
- **Figma rendering snapshots** — Use **scale.typography.snapshot.*** for Figma. `scale.typography.snapshot.web.desktop` uses refs like `{fontSize.type-1}`…`{fontSize.type-9}` (xxs→type-1 … 4xl→type-9) so the chain stays single-source. Tablet/mobile snapshots keep **computed** values (~0.9×, ~0.8× base). iOS snapshots are left untouched. Edit only the base scale; re-run the build to sync.

**Fluid scale for code (`clamp()`)**

- `scale.typography.web.fluid.fontSize.*` (t-shirt keys): **max** maps back to the primitive scale (`{fontSize.type-1}`…`{fontSize.type-9}`); **min** stays computed (mobile-scale rem). Build script writes min from base and writes max as refs so fluid stays single-source.
- Code can map to `clamp(minRem, preferred, maxRem)` using `primitives.breakpoints`.
...
- Never ship snapshot tokens to production code; exports and adapters should read from `scale.typography.base` / `scale.typography.web.fluid` and `semantics.web.typography.*` / `semantics.ios.typography.*` instead.
...
- **color semantics** — Sets `semantics.color.web.light`, `semantics.color.web.dark`, `semantics.color.ios.light`, `semantics.color.ios.dark` (background, surface, text, border, etc.), including list/table row dividers. Border color lives in these sets; `semantics.web` has border focus ring only, not border color.
- **semantics.ios** — **Separate token set** (Token Studio: turn on for iOS, off for web). `interactive.minimumTouchTarget` (44pt), `inset.safeArea.top/bottom/left/right` (pt). Use in Figma for iOS frames; disable when designing web-only.
- **scale.typography.snapshot.ios.phone / .ios.tablet** — **Separate token sets** (Token Studio: enable with **semantics.ios** for iPhone/iPad). `fontSize.*` in pt (11–40). See [IOS_TYPESTYLES.md](IOS_TYPESTYLES.md).
